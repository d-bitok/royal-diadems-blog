...
import { FeaturedPostCardPreview } from "../components";
...
const FeaturedPostsPreview = () => {
  ...
  const customLeftArrow = (
    <div className="absolute arrow-btn left-0 text-center py-3 cursor-pointer bg-green-600 rounded-full">
      <svg
        ...
        className="h-6 w-6 text-white ml-3 w-full"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
      >
  ...
  const customRightArrow = (
    <div className="absolute arrow-btn right-0 text-center py-3 cursor-pointer bg-green-600 rounded-full">
      <svg
        xmlns="http://www.w3.org/2000/svg"
        className="h-6 w-6 ml-3 text-white w-full"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
      >
      ...
      >
        {dataLoaded &&
          featuredPosts.map((post, index) => (
            <FeaturedPostCardPreview key={index} post={post} />
          ))}
      </Carousel>
    </div>
  );
